...
## @tutorial: https://rakugoteam.github.io/advanced-text-docs/3.0.1/TextParser/
## TextParser is the base class for AdvancedText addon parsers.
...
## This only exists to be overridden by parser classes that inherit from it. Returns given text unchanged.
func parse(text: String) -> String:
	return text

## Replaces result in given text with replacement.
...
## Returns given path to image as BBCode img. Size must be given as "<height>x<width>".
...
## Returns given path/url to image as BBCode link. If link_text != "" it will be displayed as link text.
...
## Used to replace given `what` Regex String with `for_what` String in given text.
